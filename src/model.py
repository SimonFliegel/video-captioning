...
def create_models():
    ...
    encoder_lstm = LSTM(config.latent_space, return_state=True, return_sequences=True, name='encoder_lstm')
    _, state_h, state_c = encoder_lstm(encoder_inputs)
    encoder_states = [state_h, state_c]

    # Decoder
    decoder_inputs = Input(shape=(None, config.num_decoder_tokens), name='decoder_inputs')
    decoder_lstm = LSTM(config.latent_space, return_sequences=True, return_state=True, name='decoder_lstm')
...
def load_inference_models():
    ...
    decoder_dense = Dense(config.num_decoder_tokens, activation='softmax', name="decoder_softmax")
    decoder_lstm = LSTM(config.latent_space, return_sequences=True, return_state=True, name='decoder_lstm')
...
def _create_decoder_inference_model(decoder_dense, decoder_inputs, decoder_lstm):
    ...
    """
    decoder_state_input_h = Input(shape=(config.latent_space,), name='decoder_state_input_h')
    decoder_state_input_c = Input(shape=(config.latent_space,), name='decoder_state_input_c')
